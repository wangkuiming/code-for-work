...
public class CaptchaServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		if (StringUtils.isBlank(suffix) || suffix.length() > 35) {
			return;
		}
		...
		drawRandomLine(g);
		// 写随机数
		String random = drawRandomNum((Graphics2D) g).toLowerCase();
		// 将随机汉字存在redis中

		RoutingXeRedis redis = new RoutingXeRedis("xeRedis");
		redis.setex(Contents.CAPTCHA_CODE_SESSION_KEY + suffix, 60 * 10,
		        MD5Util.md5(random));// 10分钟

		// request.getSession().setAttribute(Contents.CAPTCHA_CODE_SESSION_KEY,
		// random);
		// 将图形写给浏览器
